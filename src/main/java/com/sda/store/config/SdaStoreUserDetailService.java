...
public class SdaStoreUserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String s) throws UsernameNotFoundException {
        ...
        if(user == null) {
            throw new ResourceNotFoundInDatabase(String.format("The user %s is not registerd.", s));
        }
        return mapUserToUserDetails(user);
    }

    public UserDetails mapUserToUserDetails(User user){
        List<GrantedAuthority> grantedAuthorities = mapRoleToGrantedAutorithy(user.getRole());
    ...
    public List<GrantedAuthority> mapRoleToGrantedAutorithy(Set<Role> roles){
